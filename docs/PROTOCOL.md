...
| 5  | FILE_BEGIN | JSON `{"id","name","size"}`      |
...
1. Sender emits `FILE_BEGIN`
2. Sender streams `FILE_CHUNK` messages (recommended chunk size: 64 KiB)
3. Sender emits `FILE_END`
4. Receiver writes to `~/Downloads/SimpleLink/` (Mac) or `Download/SimpleLink/` (Android)
